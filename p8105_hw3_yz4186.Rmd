...
```
The "instacart" dataset has `r nrow(instacart)` rows and `r ncol(instacart)` columns. It contains data of each oder with showing the "product id", "user id" and "product name" and other detailder information. 
...
mutate( 
      weekday = 
      case_when(day %in% c("Monday","Tuesday","Wednesday","Thursday","Friday") ~ "weekday", ## generate weekday vs weekend variable
